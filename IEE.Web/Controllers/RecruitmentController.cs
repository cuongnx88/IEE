...
namespace IEE.Web.Controllers
{
    ...
    {
        private readonly IRepository<Infrastructure.DbContext.Category> _categoryRepo;
        ...
        public ActionResult Index(int? pageNum)
        {
            ...
            if (Request.IsAjaxRequest())
            {
                ...
                return PartialView("_Recruits", posts);
            }
            else
            {
                LoadAllPostToSession(24);
        ...
        {
            Dictionary<int, Post> posts = (Session["Recruits"] as Dictionary<int, Post>);
        ...
        private void LoadAllPostToSession(int? categoryId)
        {
            ...
            foreach (var post in category.PostCategories.Select(c=>c.Post).ToList())
            {
            ...
            int custIndex = 1;
            Session["Recruits"] = posts.OrderBy(t => t.OrderNumber).ThenByDescending(t => t.PublishedDate).ToDictionary(x => custIndex++, x => x);
